...
class MonoconEngine(BaseEngine):
    ...
    def build_loader(self, is_train: bool = True, use_org: bool = False):
    ...
    def evaluate_test(self) -> Dict[str, float]:
        ...
        for test_data in tqdm(self.test_loader, desc="Collecting Results..."):
            ...
            test_data = move_data_device(test_data, self.current_device)
            counter += len(test_data['img'])
            eval_results = self.model.batch_eval(test_data)
            
            # img_meta_list.extend(test_data['img_metas'])
            for ff in ['idx', 'split', 'sample_idx', 'image_path', 'ori_shape', 'pad_shape']:
                img_meta_list[ff].extend(test_data['img_metas'][ff])
            
            for field in ['img_bbox', 'img_bbox2d']:
                eval_container[field].extend(eval_results[field])
        
        return eval_container, img_meta_list
    ...
    def visualize_test(self, 
        ...
        test_dataset, test_loader = self.build_loader(is_train=False, use_org=use_org)
        ...
        visualizer = Visualizer(test_dataset, vis_format=vis_container, scale_hw=scale_hw)
        draw_item_to_func = {
            '2d': 'plot_bboxes_2d',
            '3d': 'plot_bboxes_3d',
            'bev': 'plot_bev'}
        ...
        for draw_item in draw_items:
            ...
            for idx in tqdm(range(len(test_dataset)), desc=f"Visualizing '{draw_item.upper()}'..."):
                draw_func = getattr(visualizer, draw_item_to_func[draw_item])
                
                ori_filename = os.path.basename(test_dataset[idx]['img_metas']['image_path'])
